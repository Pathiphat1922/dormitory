...
export default function AdminDashboard() {
  ...
  const currentInvoices: Invoice[] = useMemo(() => {
    return rooms
      ...
      .map(r => ({
        id: generateBillId(r.id), // เรียกใช้ฟังก์ชันสุ่มเลขบิล
        roomId: r.id,
        amount: r.price,
        dueDate: '2024-11-30',
        status: r.paymentStatus === 'overdue' ? 'pending' : (r.paymentStatus === 'paid' ? 'paid' : 'pending'), 
      }));
  }, [rooms]); // คำนวณใหม่เฉพาะตอนข้อมูลห้องเปลี่ยน

  const allInvoices = [...currentInvoices, ...historyInvoices];
          ...
          <>
            <div className="flex justify-between items-center mb-8">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                ...
                <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex items-center justify-between">
                    <div><p className="text-gray-500 text-sm font-medium">ค้างชำระ</p><p className="text-3xl font-bold text-red-600 mt-2">{rooms.filter(r => r.paymentStatus === 'overdue').length}</p></div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex font-sans text-gray-900">
      <aside className="w-64 bg-white border-r border-gray-200 hidden lg:flex flex-col fixed inset-y-0 z-10">
        ...
        <div className="p-4 border-t border-gray-100"><button onClick={handleLogout} className="w-full flex items-center gap-2 text-red-500 hover:bg-red-50 p-3 rounded-lg transition-colors font-medium"><IconLogOut /> ออกจากระบบ</button></div>
      </aside>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-fade-in relative">
                ...
                <div className="p-8 text-center">
                    ...
                    <div className="bg-gray-50 rounded-xl p-4 border border-gray-200 mb-6"><p className="text-xs text-gray-500 uppercase font-bold tracking-wider mb-1">ยอดที่ต้องชำระ</p><p className="text-3xl font-bold text-gray-800">฿{verifyingInvoice.amount.toLocaleString()}.00</p></div>
